    ...
    public class TemplateGenerator
    {
        public static string GetHTMLString(DlhistoryModel dLHistoryModel)
        {
            ...
            sb.Append(@"
                        ...
                        <p class=""generaltextclass"">Report Date:");
            sb.AppendFormat(@"{0} </p>", DateTime.Now.ToString("yyyy/MM/dd"));
            ...
            if (dLHistoryModel != null)
            {
                sb.AppendFormat(@"<tr class=""tableborderbottom"">
                            ...
                            <td  class=""tableborderright"" colspan=4>{1}</td> 
                            <td>{2}</td>
                          </tr>", dLHistoryModel.LicenseNumber, dLHistoryModel.FullName, dLHistoryModel.Dob);
            }
            sb.Append(@"
                          <tr>
            ...
            if (dLHistoryModel != null)
            {
                sb.AppendFormat(@"
                          <tr>
                            <td colspan=6>{0} </td>
                          </tr>", dLHistoryModel.Conditions?.Replace(Environment.NewLine, "<br>"));
            }
            sb.Append(@"
                        </table>
            ...
            if (dLHistoryModel != null && dLHistoryModel?.historyInfo != null && dLHistoryModel?.historyInfo.Count > 0)
            {
                ...
                foreach (var item in dLHistoryModel.historyInfo)
                {
                    sb.AppendFormat(@"<tr class=""tableborder"">
                                  ...
                                  </tr>", item.IssueDate, item.ServiceType, item.LicClass);
                }
                sb.Append(@"
                                </table>");
            }
